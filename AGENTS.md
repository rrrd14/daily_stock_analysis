...
- `.agents/` 是本地 agent 脚手架，不作为规则真源入库（`.gitignore` 已忽略）。`.agents/skills/` 必须由 `scripts/sync_agent_skills.py` 从 `.claude/skills/` 生成，禁止手工编辑；`scripts/check_ai_assets.py` 会拒绝入库的 `.agents` 文件并校验镜像是否漂移。新增其他 agent 专用目录时同样必须先明确单一真源，再通过脚本或镜像同步；禁止手工长期维护多份同义内容。
- 修改 AI 协作治理资产时，执行：

...
### 5. 部署前如何验证镜像可用（Docker E2E 冒烟测试）

在正式部署前，可运行仓库内置的 Docker E2E 冒烟测试，在真实镜像中验证「构建 → 启动 → 健康检查 → 关键只读接口 → 前端静态资源 → 非 root 用户与数据目录可写」全链路：

```bash
./scripts/docker_e2e.sh
```

该脚本不依赖任何外部 LLM / 数据源 / 通知配置，使用 Docker **命名卷**承载 `data/logs/reports`（启动前显式把属主初始化为镜像运行时用户 `1000:1000`，因此不依赖调用者宿主机 UID，Linux CI runner 同样可写），不会污染仓库工作区；失败时以非零退出码结束，可直接用于本地自检或 CI。脚本会校验 `/app/data`、`/app/logs`、`/app/reports` 三个目录均可写。可选环境变量：

| 变量 | 默认值 | 说明 |
|------|--------|------|
| `DOCKER_E2E_IMAGE` | `stock-analysis:e2e` | 构建的镜像名 |
| `DOCKER_E2E_PORT` | `18000` | 宿主机映射端口 |
| `DOCKER_E2E_KEEP` | `0` | 设为 `1` 时保留容器与命名卷，便于排障 |

CI 中该测试作为 `docker-e2e` 作业运行（依赖 `docker-build`）。

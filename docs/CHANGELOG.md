...
- [修复] ETF 长历史优先 AkShare，覆盖起点不足时继续寻找更完整来源；Polygon 接受 DELAYED 行情并修复日期列标准化；Docker 为非 root 用户授权 Efinance 数据缓存目录。

- [修复] Agent 回测查询默认跟随配置窗口，未完成评估可自动重试，汇总缺失时可从结果恢复查询。
- [新功能] 基于新报告的明确单技能归属提供技能回测汇总，排除无法归属的旧记录与联合决策。
- [改进] Agent 历史行情支持最多 1260 个交易日，长窗口校验缓存覆盖并返回日期范围与覆盖状态。
...
- [修复] 未注册的引擎类型改为显式拒绝：`engine_kind` 不在（内置 `ai_report_evaluation` + 注册表里的 `daily_return`）内一律在入口抛错，不再「换个标签跑旧报告引擎」；报告评估运行证据的 `snapshot` 块新增 `consumed=false`，并在 `limitations` 中写明引用不等于消费（策略引擎从冻结 bars 计算，`consumed=true`）。
